...
(function()
{
   ...
   extend(Asteroids.Player, Game.Actor,
   {
	  ...
	  this.heading -= 4 * GameHandler.frameMultipler;
	  },
      ...
      onRender: function onRender(ctx)
      {
         ...
         if (this.isShieldActive())//(this.shieldCounter > 0 && this.energy > 0)
         {
      ...
      {
	  //console.log("fire primary player");
         ...
         if (this.alive && (!this.isShieldActive() || this.fireWhenShield))
         {
            ...
            //{
               var b = this.primaryWeapons["main"].fire();
			   if (b){bulletList.push(b);}
               /*
			   if (b)
               {
                  if (isArray(b))
                  {
                     ...
                     {
                        bulletList.push(b[i]);
						//console.log("bulletlist.push b");
                     }
                  }
                  else
                  {
                     
					 //console.log("bulletlist.push b2");
                  }
      ...
      reset: function reset(persistPowerUps)
      {
		 ...
		 //console.log("asteroid player reset");
         this.alive = true;
         //if (true)
         //{
            this.primaryWeapons = [];
